...
void WeComWnd::setUserDetail(QString strName, QString ico, QString strEmail, QString strPart)
{
	m_pNavPane->setUserDetailDlg(strName, ico, strEmail, strPart);
}

void WeComWnd::setFriendList(const QVector<FRIENDINFO>& vFriendInfo)
{
	if (m_pMsgPane)
	{
		m_pMsgPane->setFriendsList(vFriendInfo);
	}
}
...
void WeComWnd::InitCtrl()
{
    ...
    AUTOTIP->setParent(this);   //提示窗口，设置实例的引用者
    //m_Logindlg->hide();
}
...
void WeComWnd::InitTrayIcon()
{
    ...
    m_trayIcon->setToolTip(tr("企业微信:逍遥子")); //显示提示信息 应该搞个全局变量记录当前登录者的name
    m_trayIcon->show();
}
