...
void NavPane::InitCtrl()
{
    ...
    // 初始化tab
	//QSignalMapper 作用 用于将多个相似信号（此处是按钮点击）
	//转换为带有不同参数的单个信号，简化信号处理逻辑
...
//名字 头像 邮箱 部门 电话默认
void NavPane::setUserDetailDlg(QString strName, QString ico, QString strEmail, QString strPart)
{
	if (m_pUserDetail)
	{
		m_pUserDetail->SetUserInfo(strName, ico, strEmail, strPart);
